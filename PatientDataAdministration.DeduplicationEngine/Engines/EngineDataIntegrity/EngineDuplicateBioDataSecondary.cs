            ...
            try
            {
                using (var entities = new Entities())
                {
                    ...
                    for (int i = 0; i <= records.Count(); i+= Setting.DedupDataLimit)
                    {
                        ...
                        ActivityLogger.Log("INFO", $"{TraceId}: Loaded {allSecondaryBiometrics.Count} Patients Biomatric Templates Currently");
                        ...
                        ActivityLogger.Log("INFO", $"{TraceId}: Processing is Completed at this time for batch");
                    }
